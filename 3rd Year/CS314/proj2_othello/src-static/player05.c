...
int distribute_asroot(int* board, int player, int count, int* processes, int depth) {
    ...
    if (count == 1 || sub_trees == 1) {
        ...
        g_depth = depth;
        //printf("Process %d starting with a = %d b = %d\n", rank, g_alpha, g_beta);
        return alphabeta(board, player, g_alpha, g_beta, depth, 8);
    ...
    if (rank == 0) {
        ...
        int opp = opponent(player), close = 1, next_depth = depth + 1, val, 
            top_move = 0, s_alpha, s_beta;
