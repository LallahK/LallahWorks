...
int alphabeta(int* board, int player, int alpha, int beta, int depth, int max_depth);
...
void Build_mpi_type(int* board, int* player, int* count, int* processes, int* depth, 
    MPI_Datatype* send_root); 

int distribute_asroot(int* board, int player, int count, int* processes, int depth);
...
int randomstrategy() {
    ...
    moves = legalmoves(board, my_colour);
    if (moves[0] == 0){
        return -1;
    }
    srand (time(NULL));
...
void run_worker(){
    ...
    while (1) {
        ...
        if (running == 1) {
            ...
            MPI_Datatype recv_root;
            MPI_Status recv_status;

            Build_mpi_type(board, &player, &count, processes, &depth, &recv_root);
            
            while (1) {
                MPI_Recv(board, 1, recv_root, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD, &recv_status);

                if (board[0] == -1) break;  
                val = distribute_asroot(board, player, count, processes, depth);
                MPI_Send(&val, 1, MPI_INT, recv_status.MPI_SOURCE, depth, MPI_COMM_WORLD);
            }
...
int start_dts(int size) {
    ...
    int move = distribute_asroot(board, my_colour, size, processes, 1);
#ifdef DEBUG
    fprintf(fp, "FINAL MOVE = %d\n", move);
    fflush(fp);
#endif
...
int distribute_asroot(int* board, int player, int count, int* processes, int depth) {
    ...
    if (count == 1 || sub_trees == 1) {
        if (depth == 1) {
            return moves[1];
        }
        return alphabeta(board, player, 0, 64, depth, 7);

    } else if (sub_trees > 0) {
        int check = 0, groups = 0, *process_counts, **process_blocks, *c_board, 
            **process_moves = NULL;
...
#ifdef DEBUG
    if (rank == 0) {
        fprintf(fp, "sub_trees = %d\n", sub_trees);
        for (int i = 0; i < sub_trees; i++) {
            fprintf(fp, "process_counts[%d] = %d : proccesses", i, process_counts[i]);
            for (int j = 0; j < process_counts[i]; j++) {
                fprintf(fp, " %d", process_blocks[i][j]);
            }
            fprintf(fp, "\n");
        }
        fflush(fp);
...
#endif
        for (int i = 0; i < sub_trees; i++) {
            if (process_counts[i] != 0) groups++;
        }
        int *active = malloc(sizeof(int) * groups);
        for (int i = 0; i < groups; i++) active[i] = 0;

        if (depth == 1) {
            int len = sub_trees - groups + 1;
            if (len < 1) len = 1;
            process_moves = malloc(sizeof(int*) * groups);
            for (int i = 0; i < groups; i++) {
                process_moves[i] = malloc(sizeof(int) * len);
                for (int j = 0; j < len; j++) process_moves[i][j] = -1;
            }
        }

        int opp = opponent(player), close = 1, next_depth = depth + 1, val, top_move = 0;
        MPI_Datatype send_root;
        MPI_Status done_status;

        for (int i = 1; i < groups; i++) {
            c_board = copyBoard(board);
            makemove(c_board, moves[i], player);
        
            Build_mpi_type(c_board, &opp, &(process_counts[i]), process_blocks[i], 
                &next_depth, &send_root);
            if (depth == 1) update_processmoves(process_moves, i, moves[i]);

            MPI_Send(c_board, 1, send_root, process_blocks[i][0], 0, MPI_COMM_WORLD);
        ...
        while (close) {
            if ((check + 1) % groups == 0 && check + 1 <= sub_trees) {
                c_board = copyBoard(board);
                makemove(c_board, moves[check + 1], player);
                if (depth == 1) update_processmoves(process_moves, 0, moves[check + 1]);

                val = distribute_asroot(c_board, opp, process_counts[0], process_blocks[0], 
                    next_depth);
                
                free(c_board);
                if (depth == 1) top_move = update_topmove(process_moves, 0, top_move, val, max);
                if (depth % 2 == 0) {
                    if (val < max) max = val;
                } else {
                    if (val > max) max = val;
            ...
            MPI_Recv(&val, 1, MPI_INT, MPI_ANY_SOURCE, next_depth, MPI_COMM_WORLD, &done_status);
                       
            for (int i = 0; i < groups; i++) {
                if (process_blocks[i][0] == done_status.MPI_SOURCE)
                    n = i;
            }
            if (depth == 1) top_move = update_topmove(process_moves, n, top_move, val, max);
            if (depth % 2 == 0) {
                if (val < max) max = val;
            } else {
                if (val > max) max = val;
            ...
            if (check < sub_trees) { 
                ...
                Build_mpi_type(c_board, &opp, &(process_counts[n]), process_blocks[n],
                    &next_depth, &send_root);
                if (depth == 1) update_processmoves(process_moves, n, moves[check + 1]);

                MPI_Send(c_board, 1, send_root, process_blocks[n][0], 0, MPI_COMM_WORLD);
                free(c_board);
                MPI_Type_free(&send_root);
        ...
        free(moves);
        free(process_counts);
        for (int i = 0; i < sub_trees; i++) free(process_blocks[i]);
        free(process_blocks);
        free(active);
        
        if (depth == 1) {
            for (int i = 0; i < groups; i++) free(process_moves[i]);
            free(process_moves);
            return top_move;
        }
        return max;
    } else return evalBoard(my_colour, board);
}
...
int alphabeta(int* board, int player, int alpha, int beta, int depth, int max_depth) {
    ...
    if (depth == max_depth || sub_trees == 0) {
        eval = evalBoard(my_colour, board);
        return eval;
    }
    ...
    if (my_colour == player) {
        ...
        for (int i = 1; i <= sub_trees; i++) {
            ...
            makemove(virtual_board, moves[i], player);
            alphabeta_val = alphabeta(virtual_board, opp, alpha, beta, depth + 1, max_depth);
            free(virtual_board);
...
void Build_mpi_type(int* board, int* player, int* count, int* processes, int* depth, 
    ...
    MPI_Aint displacements[5] = {0, 1, 1, 1, 1};
    int blocklengths[5] = {100, 1, 1, MAX_PROC, 1};
    ...
    MPI_Type_create_struct(5, blocklengths, displacements, types, send_root);
