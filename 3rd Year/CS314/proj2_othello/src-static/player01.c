...
#ifdef DEBUG
    if (rank == 0) {
        fprintf(fp, "(%d) subtrees %d local_size %d\n", rank, sub_trees, local_size);
        fflush(fp);
    }
#endif
...
#endif
        minmax_val = minmax(board, moves[i], my_colour, 1, 3);
...
#ifdef DEBUG
    if (rank == 0) {
    fprintf(fp, "FINAL CHOICE: %d %d\n", minmax_choice[0], minmax_choice[1]);
    fflush(fp); }
#endif
...
int minmax(int* board, int move, int player, int depth, int max_depth) {
    int eval, *moves, minmax_val, minmax_choice, *virtual_board, sub_trees, opp;
    ...
    if (depth == max_depth) {
        ...
        fprintf(fp, "Depth: %d | Player %d | Value: %d\n", depth, player, eval); }
#endif
        return eval;
    }
    ...
    if (my_colour == opp) {
        ...
        for (int i = 1; i <= sub_trees; i++) {
            minmax_val = minmax(virtual_board, moves[i], opp, depth + 1, max_depth);
            if (minmax_val >= minmax_choice) {
                minmax_choice = minmax_val;
            }
        }
    } else {
        minmax_choice = 64;
