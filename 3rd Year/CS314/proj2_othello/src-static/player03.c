...
int* alphabeta_init(int* moves, int block);
...
int awaken_threads() {
    int size, *all_moves, block_size, *moves, *alphabeta_move, move, val;

    MPI_Bcast(board, BOARDSIZE, MPI_INT, 0, MPI_COMM_WORLD);
    MPI_Comm_size(MPI_COMM_WORLD, &size);

    moves = distribute_moves(0, &block_size, my_colour);
    alphabeta_move = alphabeta_init(moves, block_size);

    all_moves = (int*)malloc(2 * sizeof(int));
    val = alphabeta_move[0];
    move = alphabeta_move[1];
...
void run_worker(){
    int running = 1, rank, block, *moves, *alphabeta_move;
    ...
    while (1) {
        ...
        if (running == 1) {
            ...
            alphabeta_move = alphabeta_init(moves, block);

            MPI_Send(alphabeta_move, 2, MPI_INT, 0, 0, MPI_COMM_WORLD);
            free(moves);
            free(alphabeta_move);
        } else if (running == 0) {
            break;
...
int* alphabeta_init(int* moves, int block) {
    int *alphabeta_choice, alphabeta_val, i;
    int rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);

    alphabeta_choice = (int*)malloc(2 * sizeof(int));
    alphabeta_choice[0] = 0;
    alphabeta_choice[1] = -1;
...
#endif
            alphabeta_val = alphabeta(board, moves[i], my_colour, g_alpha, g_beta, 1, 7);

            if (alphabeta_val >= alphabeta_choice[0]) {
                alphabeta_choice[0] = alphabeta_val;
                alphabeta_choice[1] = moves[i];
            }
            g_alpha = alphabeta_choice[0];
        }
        share_alphabeta(rank);
    }
...
int alphabeta(int* board, int move, int player, int alpha, int beta, int depth, int max_depth) {
    ...
    int rank, *send_buffer = (int*)malloc(2 * sizeof(int));
    send_buffer[0] = -1;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Request req;
    MPI_Status status;
...
void share_alphabeta(int rank) {
    int size, *vals, outcount;
    MPI_Comm_size(MPI_COMM_WORLD, &size);

    global_vals[rank][0] = g_alpha;
    global_vals[rank][1] = g_beta;
    for (int i = 0; i < size; i++) {
        MPI_Ibcast(global_vals[i], 2, MPI_INT, i, MPI_COMM_WORLD, &(requests[i]));
    }

    vals = malloc(sizeof(int) * size);

    MPI_Waitsome(size, requests, &outcount, vals, statuses);

    for (int i = 0; i < outcount; i++) {
        if (global_vals[vals[i]][0] > g_alpha) {
            g_alpha = global_vals[vals[i]][0];
        }
        if (global_vals[vals[i]][1] > g_beta) {
            g_beta = global_vals[vals[i]][1];
        }
    }
}
...
void start_share() {
    ...
    global_vals = malloc(sizeof(int*) * size);
    for (int i = 0; i < size; i++) {
        global_vals[i] = malloc(sizeof(int) * 2);
    }

    requests = malloc(sizeof(MPI_Request) * size);
    statuses = malloc(sizeof(MPI_Status) * size);
}
