...
int distribute(int player, int count, int* processes, int depth);
...
void run_worker(){
    ...
    while (1) {
        ...
        if (running == 1) {
            MPI_Bcast(board, BOARDSIZE, MPI_INT, 0, MPI_COMM_WORLD);
            distribute(my_colour, size, processes, 1);
        } else if (running == 0) {
            break;
        }
    }
...
int start_dts(int size) {
    ...
    g_alpha = 0;
    g_beta = 64;
    int move = distribute(my_colour, size, processes, 1);
    printf("Best value %d\n", move);
#ifdef DEBUG
    end = MPI_Wtime();
    global_t += (end - start);
...
int distribute(int player, int count, int* processes, int depth) {
    int *moves, sub_trees, index, local_size, rank, member = 0;
    int i, n, value, *brackets, max_i;
    int max_val = (player == my_colour) ? 0 : 64;
    MPI_Status recv;
    ...
    if (sub_trees == 0) {
        ...
        for (i = 0; i < count; i++) 
            if (processes[i] == rank) member = i;

        if (count > sub_trees) {
            /*Distribute further*/
            int *team;

            local_size = count / sub_trees + 1;
            team = malloc(sizeof(int) * local_size);

            n = 0;
            index = member % sub_trees;
            for (i  = 0 ; i < count; i++) {
                if (i % sub_trees == index) {
                    team[n] = processes[i];
                    n++;
                }
            }

            brackets = makemove2(moves[index + 1], player);
            value = distribute(opponent(player), n, team, depth + 1);
            unmakemove(moves[index + 1], player, brackets);

            max_i = index + 1;
            if (member == 0) {
                for (i = 1; i < sub_trees; i++) {
                    MPI_Recv(&value, 1, MPI_INT, MPI_ANY_SOURCE, depth + 1, MPI_COMM_WORLD, &recv);
                    index = recv.MPI_SOURCE;
                    if (player == my_colour) {
                        if (value > max_val) {
                            max_val = value; 
                            max_i = index + 1;
            ...
            } else if (member < sub_trees) {
                MPI_Send(&value, 1, MPI_INT, processes[0], depth + 1, MPI_COMM_WORLD);
            }
 
            max_i = moves[max_i];
        } else {
            /*Allocate moves and minmax*/
            int *local_moves, *top = malloc(sizeof(int) * 2), *cmp;
            top[0] = max_val;

            local_size = sub_trees / count + 1;
            local_moves = malloc(sizeof(int) * local_size);

            n = 0;
            for (i = 1; i <= sub_trees; i++) {
                if ((i - 1) % count == member) {
                    local_moves[n] = moves[i];
                    n++;
                }
            }
            ...
            for (i = 0; i < n; i++) {
                brackets = makemove2(local_moves[i], player);
                value = alphabeta(player, 0, 64, depth, 8);
                unmakemove(local_moves[i], player, brackets);
            ...
            if (member == 0) {
                for (i = 0; i < count - 1; i++) {
                    MPI_Recv(cmp, 2, MPI_INT, MPI_ANY_SOURCE, depth + 1, MPI_COMM_WORLD, &recv);
                    if (player == my_colour) {
                        if (max_val > cmp[0]) {
                            max_val = cmp[0];
                            max_i = cmp[1];
                ...
                MPI_Send(top, 2, MPI_INT, processes[0], depth + 1, MPI_COMM_WORLD);
            }
    ...
    if (rank == 0 && depth == 1) return max_i;
    return max_val;
}
