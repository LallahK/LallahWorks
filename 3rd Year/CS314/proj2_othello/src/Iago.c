...
int alphabeta_iterative(int player, int alpha, int beta, int local_depth, int max_depth);
int time_out();
    ...
    if (rank == 0){
        ...
        while (running == 1){
            ...
            if (strcmp(cmd, "game_over") == 0){
                ...
                /*Cleans up Zobrist hash table and reads all unread messages*/
                cleanup_zobrist();
                cleanup();
                printboard();
...
int start_dts() {
    ...
    while (!terminated()) {
        ...
        if (depth > cutoff) {
            tree_value = alphabeta_iterative(player, global_alpha, global_beta, depth, MAX_DEPTH);
        } else {
            local_shared = shared;
            tree_value = branch();
            shared = local_shared;
...
int terminated() {
    ...
    if (stack_size - layers[layer] >= MIN_SHARE) {
        fulfill();
        return 0;
    } else {
        reject();
    ...
    for (i = 0; i < size && !done; i++) {
        if (i != rank) {
            ...
            if (msg_avail) {
                ...
                if (recv == 0) {
                    count = (stack_size - layers[layer]) / 2;
...
int alphabeta_iterative(int player, int alpha, int beta, int local_depth, int max_depth) {
    int deep, best_move;
    best_move = (player == my_colour) ? MIN_ALPHA : MAX_BETA;

    for (deep = 6; deep < 15 && !time_out(); deep++) {
        best_move = alphabeta(player, alpha, beta, local_depth, deep);
    }
