...
void return_value(int max, int best_move) {
    ...
    printf("Process %d returning %d (%d) to process %d\n", rank, max, best_move, parent);
            ...
            if (msg_avail) {
                if (!done) {
                    ...
                    *available = 1;
                    ...
                    printf("++ %d Recieved job size %d from %d depth %d\n", rank, stack_size, parent, depth);
                } else {
                    MPI_Recv(0, 0, MPI_INT, i, WORK_AVAIL, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    MPI_Send(&done, 1, MPI_INT, i, HANDSHAKE, MPI_COMM_WORLD);                   
                }
