...
int start_dts() {
    ...
    int player = my_colour, max, best_move, locally_shared;
    ...
    while (!terminated()) {
        ...
        if (depth > cutoff) {
            ...
            locally_shared = shared;
            tree_value = branch();
            shared = locally_shared;
        }
        unmakemove(move, player, brackets);

        //printf("Rank %d tree value %d - %d\n", rank, tree_value, max);
        if (player == my_colour) {
            if (tree_value > max) {
                max = tree_value;
                best_move = move;
        ...
        if (stack_size == 0) {
            ...
            printf("process %d got %d depth %d\n", rank, max, depth);
        }
    }
...
int branch() {
    ...
    if (moves[0] == 0) return evalBoard(my_colour);
...
void get_value(int* max, int* best_move) {
    ...
    for (i = 0; i < size; i++) {
        if (i != rank) {
            MPI_Iprobe(i, RETURN, MPI_COMM_WORLD, &msg_avail, MPI_STATUS_IGNORE);
            if (msg_avail) {
                MPI_Recv(buff, 1000, MPI_PACKED, i, RETURN, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void return_value(int max, int best_move) {
    ...
    printf("Return %d (%d) to %d\n", max, best_move, parent);
    ...
    MPI_Bsend(buff, pos, MPI_PACKED, parent, RETURN, MPI_COMM_WORLD);
}
        ...
        if (i != rank) {
            ...
            if (msg_avail) {
                ...
                if (recv == 0) {
                    ...
                    //printf("-- %d Sending %d to process %d depth %d energy %.10f\n", rank, count, i, depth, energy);
                    shared++;
                    done = 1;
                } 
            }
        ...
        if (i != rank) {
            ...
            if (msg_avail) {
                if (!done) {
                    ...
                    energy = energy + val;
                    //printf("++ %d Recieved job size %d depth %d energy %.10f\n", rank, stack_size, depth, energy);
                    global_alpha = MIN_ALPHA;
                    global_beta = MAX_BETA; 
