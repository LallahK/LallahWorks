...
struct processQueue* waiting_queue();
struct processQueue* terminated_queue();

void dealloc_page(struct page *p);
void dealloc_resourceList(struct resourceList *r);
void dealloc_mailboxes();
void dealloc_queues();
void dealloc_Q(struct processQueue *q);
...
struct processQueue *readyQueue = NULL;
struct processQueue *waitingQueue = NULL;
struct processQueue *terminatedQueue = NULL;
 ...
 */
struct processQueue* ready_queue() {
  struct processQueue *startPcb;

  if (readyQueue == NULL) {
     startPcb = malloc(sizeof(struct processQueue));
     startPcb->next = NULL;
	 readyQueue = startPcb;
  }
 ...
 */
void dealloc_queues() {
  dealloc_Q(readyQueue);
  dealloc_Q(waitingQueue);
  dealloc_Q(terminatedQueue);
}
