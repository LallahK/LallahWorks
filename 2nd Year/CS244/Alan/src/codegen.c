/**
 * @file    codegen.c
 * @brief   The code generator for ALAN-2019.
...
void gen_call(char *fname, IDprop *idprop)
{
	...
	strcpy(filen, fname);
	strcat(strcat(strcat(fpath, "."), filen), "(");
	for (i = 0; i < idprop->nparams; i++) {
		base_type = idprop->params[i];
        SET_BASE_TYPE(base_type);
		switch (base_type) {
			case 2: if (!IS_ARRAY(idprop->params[i])) {
						strcat(fpath, "Z");
					} else {
						strcat(fpath, "[I");
					}
				break;
			case 4: if (!IS_ARRAY(idprop->params[i])) {
						strcat(fpath, "I");
					} else {
						strcat(fpath, "[I");
					}
				break;
			default: strcat(fpath, "V");
		}
	}
	strcat(fpath, ")");
	base_type = idprop->type;
	SET_RETURN_TYPE(base_type);
	switch (idprop->type)
	{
		case 2: if (!IS_ARRAY(idprop->type)) {
					strcat(fpath, "Z\0");
				} else {
					strcat(fpath, "[I\0");
				}
			break;
		case 4: if (!IS_ARRAY(idprop->type)) {
					strcat(fpath, "I\0");
				} else {
					strcat(fpath, "[I\0");
				}
			break;
		default: strcat(fpath, "V\0");
	}
...
static void ensure_space(int num_instr)
{
	...
	if (ip + num_instr > code_size) {
		...
		ncode = emalloc(sizeof(Code) * code_size);
		for (i = 0; i < ip; i++) {
			ncode[i] = code[i];
		}
...
static void dump_method(FILE *file, Body *b)
{
	...
	if (strcmp(b->name, "main") == 0) {
		...
		/* TODO */
		meth = emalloc(strlen(b->name) +
			(5 + 2 * idprop->nparams) * sizeof(char));
		/* TODO: build fpath appropriately */
		strcpy(meth, b->name);
		strcat(meth, "(");
		for (k = 0; k < b->idprop->nparams; k++) {
			base_type = b->idprop->params[k];
			SET_BASE_TYPE(base_type);
			switch (base_type)
			{
				case 2: if (!IS_ARRAY(b->idprop->params[k])) {
							strcat(meth, "Z");
						} else {
							strcat(meth, "[I");
						}
					break;
				case 4: if (!IS_ARRAY(b->idprop->params[k])) {
							strcat(meth, "I");
						} else {
							strcat(meth, "[I");
						}
					break;
				default: strcat(meth, "V");
			}
		}
		strcat(meth, ")");
		base_type = b->idprop->type;
        SET_RETURN_TYPE(base_type);
		switch (base_type)
		{
			case 2: if (!IS_ARRAY(b->idprop->type)) {
						strcat(meth, "Z\0");
					} else {
						strcat(meth, "[I\0");
					}
				break;
			case 4: if (!IS_ARRAY(b->idprop->type)) {
						strcat(meth, "I\0");
					} else {
						strcat(meth, "[I\0");
					}
				break;
			default: strcat(meth, "V\0");
		}
